...
install:
	...
	install -d "$(APPDIR)/assets" "$(DESTDIR)/usr/src/mechrevo-osd-0.1.1"
	install -m 644 $(SOURCES) "$(APPDIR)/"
	install -m 644 assets/*.png "$(APPDIR)/assets/"
	install -m 644 packaging/bind-events.py "$(APPDIR)/"
	install -m 644 $(addprefix driver/,$(DRIVER_SOURCES)) "$(DESTDIR)/usr/src/mechrevo-osd-0.1.1/"
...
uninstall:
	...
	rm -f $(foreach file,$(DRIVER_SOURCES),"$(DESTDIR)/usr/src/mechrevo-osd-0.1.1/$(file)")
